...
MODULE_CONTRACT_PATH = REPO_ROOT / "interface" / "copd-epidemiology-core.module.contract.v1.json"
DEFAULT_TEMPLATE_ID = "copd_baseline"
TEMPLATE_ALIASES = {
...
def materialize_template(*, template_id: str, scenario_id: str | None, country: str, start_year: int, end_year: int, output_dir: Path, overrides: list[str]) -> dict[str, Any]:
    ...
    materialized = {
        "metadata": {
            ...
            "name": f"ncd-copd materialized run input: {scenario_id or resolved_template_id}",
        ...
        "runtime_status": {
            ...
            "reason": "COPD declares demographic substrate inputs and opening-balance seeding. This materialized model has the COPD template applied, but it is not a complete proof run until the compiler lowers demographic inputs and COPD opening balances.",
        },
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Expose ncd-copd parameter registry, templates, and template materialisation helpers.")
